/* 
 * turn on led2 if expectd voltage match on A1 (pin 1.1)
 *
 * REMEMBER: unplug UART jumpers
 */
  ...
  
{
	...
	;
	ADC10AE0 |= BIT1; 	// configure pin 1.1 (A1) to analog input
	ADC10CTL0 |= ADC10ON;	// turn on ADC10
	ADC10CTL0 |= ENC; 	// enable conversion
	

	//toogle led at every input t
	const unsigned int EXPECTED_VOLTAGE 		= 115; // mV, expected tension on pin 1.1 (A1)
	...
	const float ERROR = 0.25; // error at plus or minus
